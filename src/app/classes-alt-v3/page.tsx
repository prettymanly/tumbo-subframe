      ...
      {visibleTags.map((tag, idx) => {
        ...
        return (
          <button key={tag.label} onClick={() => setActiveTags((prev) => { const next = new Set(prev); if (next.has(tag.label)) next.delete(tag.label); else next.add(tag.label); return next; })} style={{
            flexShrink: 0, padding: size === "sm" ? "6px 12px" : "6px 14px", borderRadius: "100px",
            border: isActive ? "none" : "1.5px solid rgba(0,0,0,0.15)",
            background: isActive ? pillColor : "transparent",
            color: isActive ? "#fff" : "#000",
            fontSize: size === "sm" ? "12px" : "13px", fontWeight: isActive ? 700 : 500,
    ...
    <div style={{ fontFamily: PAGE_FONT, background: "#FDFBF7", minHeight: "100vh", WebkitFontSmoothing: "antialiased" }}>
      ...
      <div className="lg:hidden flex flex-col min-h-screen">
        ...
        <header style={{
          ...
          borderBottom: "1px solid rgba(0,0,0,0.08)", padding: "16px 20px 0",
        }}>
          {/* Search bar */}
          <div style={{
            ...
            <input type="text" placeholder="Search classes, providers..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} style={{
              flex: 1, border: "none", outline: "none", fontSize: "13px", padding: "11px 0",
              background: "transparent", color: "#111", fontFamily: "inherit",
            }} />
            {inputValue && (
              <button onClick={() => setInputValue("")} style={{ padding: "0 14px", background: "transparent", border: "none", cursor: "pointer", color: "#aaa", fontSize: "16px" }} aria-label="Clear search">×</button>
            )}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", paddingBottom: "12px" }}>
            <button onClick={() => setSidebarOpen(true)} style={{
              flexShrink: 0, display: "flex", alignItems: "center", gap: "6px", padding: "7px 14px",
        ...
        {/* Mobile active chips — sticky below header */}
        {isFilterMode && hasActiveChips && (
          <div style={{ position: "sticky", top: 56, zIndex: 45, background: "#FDFBF7", borderBottom: "1px solid rgba(0,0,0,0.06)", padding: "10px 20px", display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: "10px", color: "rgba(0,0,0,0.4)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>Active:</span>
        ...
        <main style={{ flex: 1, padding: "20px 20px 80px" }}>
          <div style={{ fontSize: "11px", color: "rgba(0,0,0,0.35)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "16px" }}>
